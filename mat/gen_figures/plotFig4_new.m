...
%normROI = [-40 -35.4 0 0 33.93 45];
%normROI = [-40 -35.4 0 0 33.93 45]; % 45� slice - postsulcusandlip2 - posterior part of sulcus plus little lip
%ax_zoom = [-38.6 -32.0057 -19.8599 -13.6250 33.93-1 44.6475]; % for plane 45� - deeper
title_str = {'L2/3 PC','L4 LBC','L5 PC'}; 
%% Load data
layersP = loadLayers(layer_set_num,0); 
...
%plane = [-35.3816  -29 74.3816 -0.5 0.5 0];
%plane = [-32 -29 73 0.33 -0.67 0.67];
plane = [-45.2 -28.9 81.4 -0.5 0.5 0];% Slice analysis plane
uvROI = [0.235 2 0 1.765];
...
for c = 1:length(cell_ids)            
    ...
    %plot3(slices(6).slice_line(:,1),slices(6).slice_line(:,2),slices(6).slice_line(:,3),'Color',bcol,'LineWidth',blw);
    view(plane(4:6)); axis equal; axis off; axis tight; 
    %axis(ax_zoom); 
    ax = gca;
    opts.ax = ax;
    plotActTermsOnMorphs2(cell_id,cell_layer,nrn_model_ver,nrn_pop,...
        model_prefixPA,model_prefixPA, opts);
    plotActTermsOnMorphs2(cell_id,cell_layer,nrn_model_ver,nrn_pop,...
